...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
        ...
        #region Methods
        /// <summary>
        /// Gets an 1+1 discount (applied to order subtotal)
        /// </summary>
        /// <param name="customer">Customer</param>
        /// <param name="orderSubTotal">Order subtotal</param>
        /// <param name="appliedDiscounts">Applied discounts</param>
        /// <returns>Order discount</returns>
        protected virtual void GetOnePlusOneDiscount(Customer customer,
             out List<Discount> appliedDiscounts, IList<ShoppingCartItem> cart, out decimal DiductAmountExclTax,
            out decimal DiductAmountInclTax)
        {


            List<ShoppingCartItem> productsForDeduction = new List<ShoppingCartItem>();
            appliedDiscounts = new List<Discount>();
            int? valids = 0;
            int tempProducts = 0;
            DiductAmountExclTax = decimal.Zero;
            DiductAmountInclTax = decimal.Zero;
            if (_catalogSettings.IgnoreDiscounts)
                return;
            Dictionary<ShoppingCartItem, decimal> diductTemp = new Dictionary<ShoppingCartItem, decimal>();
            List<decimal> exlTemp = new List<decimal>();
            List<decimal> incTemp = new List<decimal>();
            decimal stubTax = new decimal();
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToSkus);
            var allowedDiscounts = new List<Discount>();
            if (allDiscounts != null)
                foreach (var discount in allDiscounts)
                {
            ...
            {
                productsForDeduction = _discountService.ValidateDiscount(discount, customer).productsForDeduction;//gets the products list for diduct
                DiductAmountExclTax = decimal.Zero;
                DiductAmountInclTax = decimal.Zero;

                foreach (var shoppingCartItem in productsForDeduction)
                {
                    //diductTemp.Add(shoppingCartItem, _priceCalculationService.GetSubTotal(shoppingCartItem));
                    decimal varIncTemp = _taxService.GetProductPrice(shoppingCartItem.Product, _priceCalculationService.GetSubTotal(shoppingCartItem), true, customer, out stubTax);
                    decimal varExlTemp = _taxService.GetProductPrice(shoppingCartItem.Product, _priceCalculationService.GetSubTotal(shoppingCartItem), false, customer, out stubTax);
                    int quant = shoppingCartItem.Quantity;
                    
                    for (int i = 0; i < quant; i++)
                    {
                        incTemp.Add(varIncTemp / quant);
                        exlTemp.Add(varExlTemp / quant);
                        tempProducts++;
                    }
                }
                incTemp.Sort();
                exlTemp.Sort();
                if (discount.MaximumDiscountedQuantity != null)
                {
                    if (tempProducts / 2 < discount.MaximumDiscountedQuantity)
                    {
                        valids = tempProducts / 2;
                    }
                    else
                    {
                        valids = discount.MaximumDiscountedQuantity;
                    }
                }
                else
                {
                ...
                for (int i = 0; i < valids; i++)
                {
                        ...
                        DiductAmountExclTax += incTemp[i];
                        DiductAmountInclTax += exlTemp[i];
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            //sub totals
            decimal DiductAmountExclTax = decimal.Zero;///////////////////////////////////////
            decimal DiductAmountInclTax = decimal.Zero;///////////////////////////////////////////
            ...
            /////////////////////// 1+1 
            GetOnePlusOneDiscount(customer, out appliedDiscounts, cart, out DiductAmountExclTax, out DiductAmountInclTax);//////////
            //subtotal with discount (excl tax)
            decimal subTotalExclTaxWithDiscount = subTotalExclTaxWithoutDiscount - discountAmountExclTax - DiductAmountExclTax;
            decimal subTotalInclTaxWithDiscount = subTotalExclTaxWithDiscount;  // its the same as last line
